...
public abstract class AbstractJoinOperator<T extends TimeEvent> extends BaseOperator implements Operator.CheckpointListener
{
  ...
  private int leftStartOffset = 0;
  private int rightStartOffset =0;
  private int leftSize = 4;
  private int rightSize = 4;
  ...
  {
    int offset = leftStartOffset;
    int size = leftSize;
    int idx = 0;
    if(!isLeft) {
      idx = 1;
      offset = rightStartOffset;
      size = rightSize;
    }
    //TimeEvent t = createEvent(tuple);
    byte[] keyBytes = tuple.getEventKey(offset, size);
    if(store[idx].put(tuple.getBytesData(), tuple.getTime(), keyBytes)) {
      join(tuple.getBytesData(), isLeft, keyBytes);
    }
  }
  /**
   ...
   */
  private void join(byte[] tuple, Boolean isLeft, byte[] keyBytes)
  {
    ...
    Object value ;
    if(isLeft) {
      value = store[1].getValidTuples(tuple, keyBytes);
    } else {
      value = store[0].getValidTuples(tuple, keyBytes);
    }
    // Join the input tuple with the joined tuples
    if(value != null) {
      ArrayList<byte[]> joinedValues = (ArrayList<byte[]>)value;
      List<T> result = new ArrayList<T>();
      for(int idx = 0; idx < joinedValues.size(); idx++) {
        T output = createOutputTuple();
        copyValue(output, tuple, isLeft);
        copyValue(output, (joinedValues.get(idx)), !isLeft);
        result.add(output);
