 ...
 * @since 2.2.0
 */
public class POJOJoinOperator<T extends TimeEvent> extends AbstractJoinOperator<T>
{
  protected Class<T> outputClass;
  ...
  private transient Constructor[] constructors = (Constructor[]) Array.newInstance(Constructor.class, 2);

  // Populate the getters from the input tuple
  @Override protected void processTuple(T tuple)
  {
    try {
  ...
  private void setAndPopulateGetters(Object tuple, Boolean isLeft) throws NoSuchMethodException
  {
    byte[] byteArray = new byte[0];
    if(isLeft && leftClass == null) {
      leftClass = tuple.getClass();
      constructors[0] = leftClass.getDeclaredConstructor(byteArray.getClass());
      populateGettersFromInput(isLeft);
    }
    if(!isLeft && rightClass == null) {
      rightClass = tuple.getClass();
      constructors[1] = rightClass.getDeclaredConstructor(byteArray.getClass());
      populateGettersFromInput(isLeft);
    }
  }
  /**
  ...
  private void populateGettersFromInput(Boolean isLeft)
  {
    ...
    for (String f : fields) {
      try {
        ...
        String getField = "get" + WordUtils.capitalize(f);
        String setField = "set" + WordUtils.capitalize(f);
        /*Class c ;
        ...
        fm.get = inputClass.getMethod(getField, null);
        fm.set = outputClass.getMethod(setField, fm.get.getReturnType());
        fieldsMap.add(fm);
      } catch (Throwable e) {
   ...
   * @return
   */
  @Override protected T createOutputTuple()
  {
    try {
   ...
   * @param isLeft
   */
  @Override protected void copyValue(T output, Object extractTuple, Boolean isLeft)
  {
    ...
    for (FieldObjectMap map : fieldsMap) {
      try {
        map.set.invoke(output, map.get.invoke(c.newInstance((Object)extractTuple), null));
      } catch (IllegalAccessException e) {
        throw new RuntimeException(e);
  ...
  {
    try {
      this.outputClass = (Class<T>) this.getClass().getClassLoader().loadClass(outputClass);
