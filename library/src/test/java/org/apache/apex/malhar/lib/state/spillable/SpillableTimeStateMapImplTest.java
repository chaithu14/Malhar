...
import org.apache.apex.malhar.lib.state.SpillableTimeStateMapImpl;
import org.apache.apex.malhar.lib.state.managed.ManagedStateTestUtils;
import org.apache.apex.malhar.lib.state.managed.TimeExtractor;
import org.apache.apex.malhar.lib.state.spillable.managed.ManagedTimeStateStore;
...
public class SpillableTimeStateMapImplTest
{
  ...
  {
    store = new ManagedTimeStateStore();
    applicationPath =  OperatorContextTestHelper.getUniqueApplicationPath(APPLICATION_PATH_PREFIX);
    ((FileAccessFSImpl)((ManagedTimeStateStore)store).getFileAccess()).setBasePath(applicationPath + "/" + "bucket_data");
  ...
  {
    SpillableTimeStateMapImpl<String, String> map = new SpillableTimeStateMapImpl<>(store,ID1,new StringSerde(), new StringSerde(), te);
  ...
  private void multiValueCheck(String[] keys, byte[] samePrefix, String[] expectedVal)
  {
    for (int i = 0; i < keys.length; i++) {
      checkValue(store, _bid(keys[i], te), keys[i], samePrefix, expectedVal[i]);
    }
  }

  private void assertMultiEqualsFromMap(SpillableTimeStateMapImpl<String, String> map, String[] expectedV, String[] keys)
  {
  ...
  private long _bid(String key, TimeExtractor te)
  {
    if (te != null) {
      return 0;
    } else {
